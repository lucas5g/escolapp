...
export function Header() {
  ...
  return (
    <header className="lg:mb-20 mb-20 z-10">
        <nav className="bg-blue-500 fixed  w-full lg:py-2 px-5 py-4 flex items-center justify-between">
          ...
          <button
            ...
            className="lg:hidden h-14 w-14 rounded text-white bg-blue-400 flex items-center justify-center transition-all border border-blue-300"
          >
            {!showMenu && <List size={40} />}
            {showMenu && <X size={40} />}

          </button>
          <button title="Mostrar meu nome" className="hidden lg:block text-white bg-blue-300 rounded-full px-3 py-2">
            LA
          </button>
        </nav>
